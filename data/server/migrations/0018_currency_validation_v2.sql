...
-- Safeguard: drop existing trigger if present to avoid duplication
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    WHERE event_object_table = 'transactions'
      AND trigger_name = 'transaction_currency_validation_insert'
  ) THEN
    DROP TRIGGER transaction_currency_validation_insert ON transactions;
  END IF;
END
$$;
...
BEGIN
  IF EXISTS (
    ...
    FROM information_schema.triggers 
    WHERE event_object_table = 'transactions'
      AND trigger_name = 'transaction_currency_validation_update'
  ) THEN
    DROP TRIGGER transaction_currency_validation_update ON transactions;
  END IF;
END
$$;
...
BEGIN
  IF EXISTS (
    ...
    WHERE event_object_table = 'accounts'
      AND trigger_name = 'account_currency_change_validation'
  ) THEN
    DROP TRIGGER account_currency_change_validation ON accounts;
  END IF;
END
$$;
