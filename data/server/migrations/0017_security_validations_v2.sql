...
BEGIN
  IF EXISTS (
    ...
    WHERE event_object_table = 'transactions'
      AND trigger_name = 'trg_validate_account_ownership'
  ) THEN
    DROP TRIGGER trg_validate_account_ownership ON transactions;
  END IF;
END
$$;
...
BEGIN
  IF EXISTS (
    ...
    FROM information_schema.triggers 
    WHERE event_object_table = 'transactions'
      AND trigger_name = 'trg_validate_category_ownership'
  ) THEN
    DROP TRIGGER trg_validate_category_ownership ON transactions;
  END IF;
END
$$;
