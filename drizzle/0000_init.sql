...
CREATE TABLE "user_preferences" (
	...
	"first_weekday" numeric DEFAULT '1',
...
CREATE UNIQUE INDEX "budgets_category_period_start_unq" ON "budgets" USING btree ("category_id","period","start_date");--> statement-breakpoint
...
CREATE POLICY "crud-authenticated-policy-select" ON "transaction_attachments" AS PERMISSIVE FOR SELECT TO "authenticated" USING (EXISTS (
        SELECT 1 FROM transactions t 
        WHERE t.id = "transaction_attachments"."transaction_id" 
        AND t.user_id = auth.user_id()
      ));--> statement-breakpoint
CREATE POLICY "crud-authenticated-policy-insert" ON "transaction_attachments" AS PERMISSIVE FOR INSERT TO "authenticated" WITH CHECK (EXISTS (
        SELECT 1 FROM transactions t 
        WHERE t.id = "transaction_attachments"."transaction_id" 
        AND t.user_id = auth.user_id()
      ));--> statement-breakpoint
CREATE POLICY "crud-authenticated-policy-update" ON "transaction_attachments" AS PERMISSIVE FOR UPDATE TO "authenticated" USING (EXISTS (
        SELECT 1 FROM transactions t 
        WHERE t.id = "transaction_attachments"."transaction_id" 
        AND t.user_id = auth.user_id()
      )) WITH CHECK (EXISTS (
...
CREATE POLICY "crud-authenticated-policy-delete" ON "transaction_attachments" AS PERMISSIVE FOR DELETE TO "authenticated" USING (EXISTS (
        SELECT 1 FROM transactions t 
        WHERE t.id = "transaction_attachments"."transaction_id" 
        AND t.user_id = auth.user_id()
      ));--> statement-breakpoint
