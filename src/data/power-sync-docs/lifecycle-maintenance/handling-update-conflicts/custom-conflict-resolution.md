...
The default behavior is "**last write wins per field**". Updates to different fields on the same record don't conflict with each other. The server processes operations in the order received, so if two users modify the _same_ field, the last update to reach the server wins.
...
async function handleUpdate(operation, userId) {
  ...
  await db.query('UPDATE tasks SET title = $1, status = $2 WHERE id = $3', [
    opData.title,
    opData.status,
    id
  ])
...
async function handleUpdateWithVersion(operation) {
  ...
  const result = await db.query('SELECT version FROM documents WHERE id = $1', [
    id
  ])
...
async function updateTaskField(
  ...
  await powerSync.writeTransaction(async tx => {
    ...
    await tx.execute(`UPDATE tasks SET ${field} = ? WHERE id = ?`, [
      newValue,
      taskId
    ])
...
**Tradeoff:** Can't show detailed server-side error messages (unless the server writes to a separate errors table that _is_ in sync rules). No long-term history on the client.
...
async function applyInventoryDelta(db, operation) {
  ...
  if (newQuantity < 0) {
    console.warn(
      `Insufficient stock: ${id} has ${currentQuantity}, attempted change: ${quantityChange}`
    )
...
await powerSync.execute(
  ...
  [-3, JSON.stringify({ operation_type: 'delta' }), inventoryId]
)
```
